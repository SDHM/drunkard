...
func ConsistentHaslLB(r naming.Resolver) grpc.Balancer {
	return &consistentHaslLB{r: r}
}

func (rr *consistentHaslLB) Start(target string, config grpc.BalancerConfig) error {
	rr.mu.Lock()
	defer rr.mu.Unlock()
	if rr.done {
...
func (rr *consistentHaslLB) watchAddrUpdates() error {
...
func (rr *consistentHaslLB) Up(addr grpc.Address) func(error) {
...
func (rr *consistentHaslLB) down(addr grpc.Address, err error) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
	for _, a := range rr.addrs {
		if addr == a.Addr {
...
func (rr *consistentHaslLB) Get(ctx context.Context, opts grpc.BalancerGetOptions) (addr grpc.Address, put func(), err error) {
	put = func() {
...
func (rr *consistentHaslLB) Notify() <-chan []grpc.Address {
	return rr.addrCh
}

func (rr *consistentHaslLB) Close() error {
	rr.mu.Lock()
	defer rr.mu.Unlock()
	if rr.done {
